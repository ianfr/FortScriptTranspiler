# Serial FortScript port of benchmarks/laplace_2d.py.
# 2D Laplace equation solved via Jacobi iteration on a uniform grid.
...
def laplace_2d(nx: int, ny: int, rtol: float, maxiter: int):
    ...
    # Compute sum(phi) as a verification check
    sum_phi: float = 0.0
    for i in range(nx):
        for j in range(ny):
